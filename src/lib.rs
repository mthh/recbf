...
pub fn recursive_bf(img: &mut [u8],
                    ...
                    channel: u32)
                    -> Result<(), &'static str> {
    if width * height * channel != img.len() as u32 {
        Err("Unexpected length for the input image. Check the width, height and channel parameters."
                .into())
    } else {
        unsafe {
            _recursive_bf_unsafe(img.as_mut_ptr(),
                                 sigma_spatial,
                                 sigma_range,
                                 width,
                                 height,
...
unsafe fn _recursive_bf_unsafe(img: *mut u8,
                               sigma_spatial: f32,
                               sigma_range: f32,
                               width: u32,
                               height: u32,
    ...
    let mut b =
        vec![0.0f32; ((width_height_channel + width_height + width_channel + width) * 2) as usize];
    let img_out_f: *mut f32 = b.as_mut_ptr();
    let img_temp: *mut f32 = &mut *img_out_f.offset(width_height_channel as (isize)) as (*mut f32);
    let map_factor_a: *mut f32 = &mut *img_temp.offset(width_height_channel as (isize)) as
                                 (*mut f32);
    ...
    let slice_factor_b: *mut f32 = &mut *slice_factor_a.offset(width_channel as (isize)) as
                                   (*mut f32);
    let line_factor_a: *mut f32 = &mut *slice_factor_b.offset(width_channel as (isize)) as
                                  (*mut f32);
    ...
    let mut alpha: f32 = (-(2.0f64).sqrt() / (sigma_spatial * width as (f32)) as (f64)).exp() as
                         (f32);
    let mut ypr: f32;
    let mut ypg: f32;
    let mut ypb: f32;
    ...
    for y in 0..height {
        let mut temp_x: *mut f32 = &mut *img_temp.offset((y * width_channel) as (isize)) as
                                   (*mut f32);
        ...
        *temp_x = ypb;
        ...
        texture_x = texture_x.offset(1isize);
        let mut temp_factor_x: *mut f32 = &mut *map_factor_a.offset((y * width) as (isize)) as
                                          (*mut f32);
        fp = 1f32;
        *temp_factor_x = fp;
    ...
    for x in 0..width {
        *map_factor_b.offset((h1 * width + x) as (isize)) =
            0.5f32 *
            (*map_factor_b.offset((h1 * width + x) as (isize)) + *ypf.offset(x as (isize)));
    }
    ycy = slice_factor_a;
    ypy = slice_factor_b;
    copy_nonoverlapping(
    ...
    for x in 0..width {
        for c in 0..channel {
            ...
            *img_out_f.offset(idx) = 0.5f32 * (*img_out_f.offset(idx) + *ypy.offset(k as (isize))) /
                                     *map_factor_b.offset((h1 * width + x) as (isize));
            k += 1;
        }
    }
    for y in (0..h1).rev() {
        ...
        let mut out_: *mut f32 = &mut *img_out_f.offset((y * width_channel) as (isize)) as
                                 (*mut f32);
